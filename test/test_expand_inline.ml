...
let _ =
  Ppx_sexp_conv_lib.Sexp.List
    ...
    ; (fun [@merlin.hide] x__001_ -> Lazy.sexp_of_t__stack sexp_of_int__stack x__001_)
        (lazy 1)
    ]
;;
...
let _ =
  ...
  Ppx_sexp_conv_lib.Sexp.List
    (Ppx_sexp_conv_lib.Sexp.Atom "one"
     ::
     (match
        (fun [@merlin.hide] x__002_ -> sexp_of_list__stack sexp_of_int__stack x__002_)
          int_list
      with
